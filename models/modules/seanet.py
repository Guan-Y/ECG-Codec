...
def pad_to_patch(x, patch_size):
    ...
    bs, ch, t = x.shape
    if t % patch_size:
        x = F.pad(x, (0, patch_size - t % patch_size), mode="reflect")
    patch_num = int(x.shape[-1] / patch_size)
    return torch.reshape(x, (bs, ch, patch_num, patch_size))
...
class SEANetEncoder(nn.Module):
    ...
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
                 ...
                 pad_mode: str = 'reflect', true_skip: bool = False, compress: int = 2, lstm: int = 1, use_patch: bool = False):
        super().__init__()
        ...
        if use_patch:
            self.patch_size = 360
            self.hidden_dim = 32
            self.projection = WNConv2d(channels, self.hidden_dim, (1, 5), (1, 2), padding=(0, 2))
        else:
        ...
        model: tp.List[nn.Module] = [
            SConv1d(self.hidden_dim, mult * n_filters, kernel_size, norm=norm, norm_kwargs=norm_params,
                    causal=causal, pad_mode=pad_mode)
        ]
    ...
    def forward(self, x):
        if self.use_patch:
            assert x.dim() == 3
            bs, _, _ = x.shape
            x = pad_to_patch(x, self.patch_size)
            x = self.projection(x)
            x = torch.reshape(x, (bs, self.hidden_dim, -1))
        return self.model(x)
...
class SEANetDecoder(nn.Module):
    ...
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
                 ...
                 norm: str = 'weight_norm', norm_params: tp.Dict[str, tp.Any] = {}, kernel_size: int = 7,
                 last_kernel_size: int = 7, residual_kernel_size: int = 3, dilation_base: int = 2, causal: bool = False,
                 pad_mode: str = 'reflect', true_skip: bool = False, compress: int = 2, lstm: int = 1,
